...
**问题 1｜这次要哪种格式？**
- A. **PDF** —— 打印、存档、发文件
- B. **卡片型 HTML** —— 分页卡片容器，适合本地浏览、发群文件、截图分享
- C. **公众号 HTML** —— 连续文流、全内联样式，粘贴进公众号编辑器直接发表
- D. **全要（PDF + 卡片型 HTML + 连续文流公众号版）**
...
规则：格式选 C 跳过问题 2（微信宽度固定）；选 D（全要）时问题 2 只作用于 PDF 与卡片型 HTML（公众号恒为连续文流、宽 677px 固定）。**画幅即形态**（style-guide §6.8）：16:9=演示形态（每页必有视觉主体，纯文字页禁止，单页 ≤350 字，真图 ≥5/图式 ≥4）；A4/长图=文章形态（图文并茂、文字多无妨）；3:4=半图半文。PPT 形态下段落型章节必须转译为"图 + 卡"叙事。**HTML 由"谁来读"决定形态**（style-guide §7.11）：发表给他人读 → 连续文流（公众号/博客型）；本地自己看 → 卡片型；用途是发表时主动建议 C 或双版本，分页卡片不适合阅读场景。
